...
class Chars2Vec:
    ...
    def __init__(self, char_to_ix, emb_dim=36):
        '''
        ...
        self.embedding_model = keras.models.Model(
            inputs=[lstm_input], outputs=x)

        # 2 modulos de entrada porque recibe 2 palabras a la entrada
        model_input_1 = keras.layers.Input(shape=(None, self.vocab_size))
        model_input_2 = keras.layers.Input(shape=(None, self.vocab_size))

        #2 el correspondiente embedding de las modulos de entrada
        ...
        ### no entiendo por que el producto punto de [x, x] no da 1 siempre ###
        ...
        self.model.compile(optimizer=keras.optimizers.Adam(
            learning_rate=0.001), loss=keras.losses.BinaryCrossentropy())

    def get_word_indexes(self, word_pairs):
        x_1, x_2 = [], []
        ...
        return x_1_pad_seq, x_2_pad_seq
    ...
    def vectorize(self, words, maxlen_padseq=None):
        ...
        if not isinstance(words, list) and not isinstance(words, np.ndarray):
            raise TypeError(
                "parameter 'words' must be a list or numpy.ndarray")
        ...
        if len(new_words) > 0:
            ...
            new_words_vectors = self.embedding_model.predict(
                [embeddings_pad_seq])
    ...
    def tester(self, test):
        ...
        test_indexes = self.get_word_indexes(test)
        pred = self.model.predict(test_indexes)
    ...
    def predict_proba(self, test):
        ...
        Args:
            self (Char2Vec): modelo Char2Vec entrenado
    ...
    def load_weights(self, path):
        ...
        '''
        path_to_model = os.path.dirname(
            os.path.abspath(__file__)) + '/trained_model/' + path
    ...
    def save_model(self, path, suffix=''):
        ...
        try:
            self.model.save_weights(
                path_to_model + '/weights' + suffix + '.h5')
